import fs from "fs";
import path from "path";

const root = process.cwd();

function write(rel, content) {
  const p = path.join(root, rel);
  fs.mkdirSync(path.dirname(p), { recursive: true });
  fs.writeFileSync(p, content.replace(/\r\n/g, "\n"));
  const lines = content.split(/\n/).length;
  console.log("wrote", rel, lines);
}

function extractLines(file, start, end) {
  const lines = fs.readFileSync(path.join(root, file), "utf8").split(/\r?\n/);
  return lines.slice(start - 1, end).join("\n");
}

function findMethod(lines, name) {
  const re = new RegExp(`^\\s*(async\\s+)?${name}\\(`);
  for (let i = 0; i < lines.length; i++) {
    if (re.test(lines[i])) return i + 1;
  }
  return -1;
}

function toExportFunction(block) {
  let b = block
    .split(/\r?\n/)
    .map(l => (l.startsWith("  ") ? l.slice(2) : l))
    .join("\n");
  b = b.replace(/^async (generate|send)/m, "export async $1");
  b = b.replace(/^(generate|send)/m, "export $1");
  return b.trim() + "\n";
}

// ---- RESEND ----
const resendSrc = "src/service/resendEmail.service.ts";
const resendLines = fs
  .readFileSync(path.join(root, resendSrc), "utf8")
  .split(/\r?\n/);

const find = name => findMethod(resendLines, name);
const methodBodyThrough = (startName, endNameOrLine) => {
  const start = find(startName);
  if (start < 0) throw new Error("missing " + startName);
  let end;
  if (typeof endNameOrLine === "string") {
    end = find(endNameOrLine) - 1;
  } else {
    end = endNameOrLine;
  }
  return extractLines(resendSrc, start, end);
};

write(
  "src/service/email/types.ts",
  `export interface EmailTemplate {
  subject: string;
  html: string;
}

export interface EmailData {
  to: string;
  subject: string;
  html: string;
  from?: string;
}
`
);

write(
  "src/service/email/resendClient.ts",
  `import { Resend } from "resend";
import nodemailer from "nodemailer";
import type { EmailData } from "./types";

const resend = process.env.RESEND_API_KEY
  ? new Resend(process.env.RESEND_API_KEY)
  : null;

function resolveResendFromEmail(): string {
  const raw =
    process.env.RESEND_FROM_EMAIL ||
    process.env.ADMIN_EMAIL_FROM ||
    "support@jevahapp.com";
  return raw.trim();
}

const fromEmail = resolveResendFromEmail();
const fromName = "Jevah Support";
const smtpTransporter = nodemailer.createTransport({
  host: process.env.SMTP_HOST || "smtp.zoho.com",
  port: parseInt(process.env.SMTP_PORT || "587", 10),
  secure: process.env.SMTP_SECURE === "true",
  auth: {
    user: process.env.SMTP_USER || "support@jevahapp.com",
    pass: process.env.SMTP_PASS || "",
  },
});
const smtpFromName = process.env.SMTP_FROM_NAME || "Jevah Support";
const smtpFromEmail = process.env.SMTP_USER || "support@jevahapp.com";

export async function testConnection(): Promise<boolean> {
  try {
    if (!process.env.RESEND_API_KEY) {
      console.log(
        "⚠️ RESEND_API_KEY is not configured - Resend service disabled"
      );
      return false;
    }
    if (!resend) {
      console.log("⚠️ Resend client not initialized");
      return false;
    }
    await resend.domains.list();
    console.log("✅ Resend connection verified successfully");
    return true;
  } catch (error) {
    console.error("❌ Resend connection failed:", error);
    return false;
  }
}

export async function sendEmail({ to, subject, html, from }: EmailData) {
  try {
    if (!resend) {
      throw new Error("Resend client not initialized - API key missing");
    }
    const response = await resend.emails.send({
      from: from || \`\${fromName} <\${fromEmail}>\`,
      to: [to],
      subject,
      html,
    });
    if (response.error) {
      throw new Error(
        \`Resend rejected email: \${response.error.message || "Unknown error"}\`
      );
    }
    console.log("✅ Email sent successfully via Resend:", {
      id: response.data?.id,
      to,
      subject,
    });
    return response;
  } catch (error) {
    console.error("❌ Resend email send failed:", error);
    try {
      console.warn("⚠️ Retrying email via SMTP fallback...", { to, subject });
      const smtpInfo = await smtpTransporter.sendMail({
        from: from || \`"\${smtpFromName}" <\${smtpFromEmail}>\`,
        to,
        subject,
        html,
      });
      console.log("✅ Email sent successfully via SMTP fallback:", {
        id: smtpInfo.messageId,
        to,
        subject,
      });
      return { data: { id: smtpInfo.messageId }, error: null };
    } catch (smtpError) {
      console.error("❌ SMTP fallback email send failed:", smtpError);
      throw new Error(
        \`Failed to send email via Resend and SMTP fallback: \${smtpError}\`
      );
    }
  }
}
`
);

write(
  "src/service/email/templates/authEmails.ts",
  `import { renderEmailTemplate } from "../../../emails/render";

export function generateVerificationEmail(firstName: string, code: string): string {
  return renderEmailTemplate("verify", { firstName, code });
}

export function generatePasswordResetEmail(firstName: string, resetCode: string): string {
  return renderEmailTemplate("reset", { firstName, resetCode });
}

export function generateWelcomeEmail(firstName: string): string {
  return renderEmailTemplate("welcome", { firstName });
}
`
);

const moderationTemplates =
  toExportFunction(
    methodBodyThrough(
      "generateContentRemovedEmail",
      "generateAdminModerationAlertEmail"
    )
  ) +
  "\n" +
  toExportFunction(
    methodBodyThrough("generateAdminModerationAlertEmail", "sendContentRemovedEmail")
  );

write("src/service/email/templates/moderationEmails.ts", moderationTemplates);

const reportTemplates =
  toExportFunction(
    methodBodyThrough(
      "generateAdminReportNotificationEmail",
      "sendAdminReportNotification"
    )
  ) +
  "\n" +
  toExportFunction(
    methodBodyThrough(
      "generateAdminCommentReportNotificationEmail",
      "sendAdminCommentReportNotification"
    )
  );

write("src/service/email/templates/reportEmails.ts", reportTemplates);

// Facade
write(
  "src/service/resendEmail.service.ts",
  `import type { EmailData, EmailTemplate } from "./email/types";
import { sendEmail as sendEmailCore, testConnection as testConnectionCore } from "./email/resendClient";
import {
  generateVerificationEmail,
  generatePasswordResetEmail,
  generateWelcomeEmail,
} from "./email/templates/authEmails";
import {
  generateContentRemovedEmail,
  generateAdminModerationAlertEmail,
} from "./email/templates/moderationEmails";
import {
  generateAdminReportNotificationEmail,
  generateAdminCommentReportNotificationEmail,
} from "./email/templates/reportEmails";

export type { EmailData, EmailTemplate };

class ResendEmailService {
  async testConnection(): Promise<boolean> {
    return testConnectionCore();
  }

  async sendEmail(data: EmailData) {
    return sendEmailCore(data);
  }

  generateVerificationEmail(firstName: string, code: string): string {
    return generateVerificationEmail(firstName, code);
  }

  generatePasswordResetEmail(firstName: string, resetCode: string): string {
    return generatePasswordResetEmail(firstName, resetCode);
  }

  generateWelcomeEmail(firstName: string): string {
    return generateWelcomeEmail(firstName);
  }

  async sendVerificationEmail(email: string, firstName: string, code: string) {
    const html = this.generateVerificationEmail(firstName, code);
    return this.sendEmail({
      to: email,
      subject: "Verify Your Email Address - Jevah",
      html,
    });
  }

  async sendPasswordResetEmail(
    email: string,
    firstName: string,
    resetCode: string
  ) {
    const html = this.generatePasswordResetEmail(firstName, resetCode);
    return this.sendEmail({
      to: email,
      subject: "Reset Your Password - Jevah",
      html,
    });
  }

  async sendWelcomeEmail(email: string, firstName: string) {
    const html = this.generateWelcomeEmail(firstName);
    return this.sendEmail({
      to: email,
      subject: "Welcome to Jevah! 🎉",
      html,
    });
  }

  generateContentRemovedEmail(
    firstName: string,
    contentTitle: string,
    reason: string,
    flags: string[] = []
  ): string {
    return generateContentRemovedEmail(firstName, contentTitle, reason, flags);
  }

  generateAdminModerationAlertEmail(
    contentTitle: string,
    contentType: string,
    uploadedBy: string,
    moderationResult: {
      isApproved: boolean;
      confidence: number;
      reason?: string;
      flags: string[];
      requiresReview: boolean;
    },
    reportCount: number = 0
  ): string {
    return generateAdminModerationAlertEmail(
      contentTitle,
      contentType,
      uploadedBy,
      moderationResult,
      reportCount
    );
  }

  async sendContentRemovedEmail(
    email: string,
    firstName: string,
    contentTitle: string,
    reason: string,
    flags: string[] = []
  ) {
    const html = this.generateContentRemovedEmail(
      firstName,
      contentTitle,
      reason,
      flags
    );
    return this.sendEmail({
      to: email,
      subject: "Content Removed - Jevah Community Guidelines",
      html,
    });
  }

  async sendAdminModerationAlert(
    adminEmails: string[],
    contentTitle: string,
    contentType: string,
    uploadedBy: string,
    moderationResult: {
      isApproved: boolean;
      confidence: number;
      reason?: string;
      flags: string[];
      requiresReview: boolean;
    },
    reportCount: number = 0
  ) {
    const html = this.generateAdminModerationAlertEmail(
      contentTitle,
      contentType,
      uploadedBy,
      moderationResult,
      reportCount
    );
    const subject = \`⚠️ Content Moderation Alert: \${contentTitle}\`;
    const results = [];
    for (const email of adminEmails) {
      results.push(await this.sendEmail({ to: email, subject, html }));
    }
    return results;
  }

  generateAdminReportNotificationEmail(
    contentTitle: string,
    contentType: string,
    reportReason: string,
    reportDescription: string | undefined,
    reporterName: string,
    reportCount: number
  ): string {
    return generateAdminReportNotificationEmail(
      contentTitle,
      contentType,
      reportReason,
      reportDescription,
      reporterName,
      reportCount
    );
  }

  async sendAdminReportNotification(
    adminEmails: string[],
    contentTitle: string,
    contentType: string,
    reportReason: string,
    reportDescription: string | undefined,
    reporterName: string,
    reportCount: number
  ) {
    const html = this.generateAdminReportNotificationEmail(
      contentTitle,
      contentType,
      reportReason,
      reportDescription,
      reporterName,
      reportCount
    );
    const subject = \`🚩 Content Report: \${contentTitle}\`;
    const results = [];
    for (const email of adminEmails) {
      results.push(await this.sendEmail({ to: email, subject, html }));
    }
    return results;
  }

  generateAdminCommentReportNotificationEmail(
    commentPreview: string,
    reportReason: string,
    reportDescription: string | undefined,
    reporterName: string,
    reportCount: number,
    mediaTitle?: string
  ): string {
    return generateAdminCommentReportNotificationEmail(
      commentPreview,
      reportReason,
      reportDescription,
      reporterName,
      reportCount,
      mediaTitle
    );
  }

  async sendAdminCommentReportNotification(
    adminEmails: string[],
    commentPreview: string,
    reportReason: string,
    reportDescription: string | undefined,
    reporterName: string,
    reportCount: number,
    mediaTitle?: string
  ) {
    const html = this.generateAdminCommentReportNotificationEmail(
      commentPreview,
      reportReason,
      reportDescription,
      reporterName,
      reportCount,
      mediaTitle
    );
    const subject = \`🚩 Comment Report\${mediaTitle ? \`: \${mediaTitle}\` : ""}\`;
    const results = [];
    for (const email of adminEmails) {
      results.push(await this.sendEmail({ to: email, subject, html }));
    }
    return results;
  }
}

export default new ResendEmailService();
`
);

console.log("resend modularization done");
