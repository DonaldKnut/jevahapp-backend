...
class SocketService {
  ...
  constructor(server: HTTPServer) {
    this.io = new SocketIOServer(server, {
      ...
      // Transport configuration - WebSocket first, then polling fallback
      transports: ["websocket", "polling"],
      // Path configuration (must match Nginx location block)
      path: "/socket.io/",
      // Connection timeouts
      pingTimeout: 60000, // How long to wait for pong before considering connection closed (60s)
      pingInterval: 25000, // How often to send ping packets (25s)
      upgradeTimeout: 10000, // How long to wait for upgrade handshake (10s)
      // Message size limit
