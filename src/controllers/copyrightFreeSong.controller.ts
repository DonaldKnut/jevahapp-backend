 ...
 * 
 * Returns a list of unique categories with counts from all copyright-free songs in the database
 */
export const getCategories = async (req: Request, res: Response): Promise<void> => {
  // Default categories fallback (always available)
  ...
  try {
    // Check if model is available and database is connected
    ...
    // Always return a successful response with default categories on error
    ...
    res.status(200).json({
      ...
      // Include error info in development only
