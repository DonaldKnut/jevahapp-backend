...
/**
 * Delete reported media content (admin only)
 * This endpoint allows admins to permanently delete content that has been reported
 */
export const deleteReportedMedia = async (
  request: Request,
  response: Response
): Promise<void> => {
  try {
    ...
    const admin = await User.findById(userId).select("firstName lastName username email");
    const adminName = admin
      ? `${admin.firstName || ""} ${admin.lastName || ""}`.trim() || admin.username || admin.email
      : "Admin";

    // Get media uploader information for notification
    const uploader = await User.findById(media.uploadedBy).select("email firstName lastName username");
    const uploaderEmail = uploader?.email;
    const uploaderName = uploader
      ? `${uploader.firstName || ""} ${uploader.lastName || ""}`.trim() || uploader.username || uploader.email
      : "User";
    ...
    if (media.fileObjectKey) {
      try {
        ...
        logger.error("Error deleting media file from storage:", error);
    ...
    await MediaReport.updateMany(
      { mediaId: new Types.ObjectId(id) },
      {
        $set: {
          ...
          reviewedAt: new Date(),
          adminNotes: `Content deleted by admin: ${adminName}`,
        },
      }
    );
    ...
    // Send notification to content creator if they exist
    if (uploader && uploaderEmail) {
      try {
        await NotificationService.createNotification({
          ...
          message: `Your content "${media.title}" has been removed due to policy violations.`,
          metadata: {
            mediaId: id,
            contentType: media.contentType,
            reason: "Admin deletion due to reports",
            deletedBy: adminName,
          },
          priority: "high",
          relatedId: id,
        ...
        logger.error("Failed to send notification to content creator:", notifError);
      }
    }

    logger.info("Reported media deleted by admin", {
      mediaId: id,
    ...
    logger.error("Delete reported media error:", error);
    response.status(500).json({
      success: false,
