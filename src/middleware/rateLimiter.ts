...
// AI Description Generation rate limiter (strict - expensive operation)
export const aiDescriptionRateLimiter =
  process.env.NODE_ENV === "development" || process.env.NODE_ENV === "test"
    ? (_req: Request, _res: Response, next: NextFunction) => next()
    : rateLimit({
        windowMs: 60 * 1000, // 1 minute
        max: 5, // limit to 5 requests per minute (cost control)
        standardHeaders: true,
        legacyHeaders: false,
        handler: (_req: Request, res: Response) =>
          res.status(429).json({
            success: false,
            message: "Too many description generation requests. Please wait a minute before trying again.",
