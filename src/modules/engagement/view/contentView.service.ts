...
async function verifyContentExists(contentId: string, contentType: ContentType): Promise<boolean> {
  try {
    switch (contentType) {
      case "media":
      case "ebook":
...
async function getViewCount(contentId: string, contentType: ContentType): Promise<number> {
  if (["media", "ebook", "podcast", "merch"].includes(contentType)) {
...
const viewService = {
  async recordView(
    ...
    if (
      !qualifiesView({ kind, durationMs, progressPct: normalizedProgress, isComplete })
    ) {
      const viewCount = await getViewCount(contentId, contentType);
      return { contentId, viewCount, hasViewed: false, counted: false };
    }
    ...
    try {
      ...
      logger.warn("Failed to emit view-updated", { contentId, error: (e as any)?.message });
    }
