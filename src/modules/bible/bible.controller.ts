...
import { resolveClientProfile } from "../clientProfile/liteProfile";
...
/**
 * Books carry `chapters`; clients render chapter grids from `chapterCount`.
 * Both are emitted so no client needs a second request.
 */
function shapeBookJson(doc: any) {
  if (!doc) return doc;
  const plain = typeof doc.toObject === "function" ? doc.toObject() : doc;
  return { ...plain, chapterCount: plain.chapters ?? null };
}
 ...
 * Whole book in one response so the reader pays RTT once per book, not per chapter.
 */
export const getBookVerses = async (
  request: Request,
  response: Response
): Promise<void> => {
  try {
    ...
    const book = await bibleService.getVersesByBook(bookName, translation);
    if (!book) {
      response.status(404).json({
        success: false,
        message: "Book not found",
    ...
    logger.error("Get book verses error:", error);
    response.status(500).json({
      success: false,
...
export const searchBible = async (
  ...
  try {
    ...
    const { results, total, hasMore } =
      await bibleService.searchBible(searchOptions);
 ...
 * Flat alias of the manifest for clients that download packs by query param.
 */
export const getPackManifestByQuery = async (
  request: Request,
  response: Response
): Promise<void> => {
  try {
    const rawId = String(request.query.translation || "");
    if (rawId && isLicensedTranslation(rawId)) {
      response.status(403).json(licenseDeniedBody());
      return;
    }
    const translation = await resolveTranslationParam(request, response);
    ...
    response.status(200).json({
      success: true,
      data: {
        translation: manifest!.translationId,
        version: String(manifest!.packVersion),
        url: manifest!.packUrl,
        sizeBytes: manifest!.bytes,
        sha256: manifest!.contentHash.replace(/^sha256-/, ""),
        license:
          manifest!.license === "public-domain" ? "Public Domain" : "Permissive",
      },
    });
  } catch (error) {
    logger.error("Get pack manifest by query error:", error);
    response.status(500).json({
      success: false,
      message: "Failed to get pack manifest",
    });
