...
/**
 * @route   DELETE /api/media/reports/:id/delete
 * @desc    Delete reported media content (admin only)
 * @access  Protected (Admin only)
 * @param   { id: string } - MongoDB ObjectId of the media item
 * @returns { success: boolean, message: string, data: { mediaId: string, mediaTitle: string, contentType: string, reportsResolved: number, deletedBy: string } }
 */
router.delete(
  "/reports/:id/delete",
  verifyToken,
