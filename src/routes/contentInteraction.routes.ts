 ...
 * @desc    Toggle like on any content type (media, artist, merch, ebook, podcast)
 * @access  Protected
 * @param   { contentType: string } - Type of content (media, artist, merch, ebook, podcast) - Note: devotional not supported
 ...
 * @desc    Add comment to content (media only - devotional comments use separate system)
 * @access  Protected
 * @param   { contentType: string } - Type of content (media)
 * @param   { contentId: string } - MongoDB ObjectId of the content
 ...
 */
router.post("/batch-metadata", verifyTokenOptional, getBatchContentMetadata);

/**
 * @route   GET /api/content/:contentType/:contentId/likers
 * @desc    Get paginated list of users who liked the content
 * @access  Public
 * @query   page?: number, limit?: number
 */
router.get("/:contentType/:contentId/likers", getContentLikers);
 ...
 * @desc    Get comments for content (media only - devotional comments use separate system)
 * @access  Public
 * @param   { contentType: string } - Type of content (media)
 * @param   { contentId: string } - MongoDB ObjectId of the content
 * @query   { page?: number, limit?: number }
 ...
 */
router.get("/:contentType/:contentId/comments", verifyTokenOptional, getContentComments);
