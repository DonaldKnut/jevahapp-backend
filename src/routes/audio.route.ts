...
router.get(
  "/copyright-free",
  apiRateLimiter,
  cacheMiddleware(30), // 30 seconds for public copyright-free songs (faster cache refresh)
 ...
 * @access  Public (No authentication required)
 * @returns 302 Redirect with Location header set to the audio URL
 */
router.get(
  "/copyright-free/:songId/stream",
  apiRateLimiter,
  cacheMiddleware(60), // 1 minute cache for redirect lookup (faster refresh)
...
router.get(
  ...
  apiRateLimiter,
  cacheMiddleware(60), // 1 minute for song details (faster cache refresh)
...
router.get(
  ...
  apiRateLimiter,
  cacheMiddleware(15), // 15 seconds for search results (faster refresh for dynamic searches)
...
router.get(
  ...
  apiRateLimiter,
  cacheMiddleware(15), // 15 seconds for search suggestions (faster refresh)
...
router.get(
  ...
  apiRateLimiter,
  cacheMiddleware(60), // 1 minute for trending searches (faster refresh)
...
router.get(
  ...
  apiRateLimiter,
  cacheMiddleware(120), // 2 minutes for categories (faster refresh, still stable)
